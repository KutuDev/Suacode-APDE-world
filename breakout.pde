...
void mousePressed() 
{
  ...
  if (mouseY < (height - 200))// this conditional statement prevents a clash between the use of mouseX by the keywords constraint & mousePressed at the same time 
  {
    ...
    if (game.playerLifes == 0 || numOfBricksHit <= totalBricks)//this conditional statement prevents the restart of game on the detection of another mousePressed while game is ON
    {
      game.restartGame();
    }
  }
